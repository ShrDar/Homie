...
import { collection, onSnapshot, addDoc, serverTimestamp, doc, updateDoc, deleteDoc, DocumentData, Timestamp } from "firebase/firestore";
...
export default function YapDuo({ session } : { session: Session }) {
    ...
    const sendMessage = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Update last message in Yap collection
    ...
    return (
        <div className="w-full h-[80dvh] z-[20] flex bg-bgSecondary text-fontPrimary rounded-[15px] p-5 flex-col justify-center items-center gap-2">
            ...
            <div className="yapsContainer w-full bg-bgPrimary h-[70dvh] overflow-auto rounded-[15px] flex flex-col justify-start items-center p-4">
                    ...
                    <div key={groupIndex} className="w-full flex flex-col gap-1 mb-4">
                        ...
                        {group.messages.map((message, messageIndex) => {
                            ...
                            return (
                                <div 
                                    key={message.id}
                                    className={`w-full flex ${message.senderId === session?.user?.id ? 'justify-end' : 'justify-start'} items-end gap-2 mb-[2px]`}
                                >
                                    {showAvatar ? (
                                        <div className="w-10 h-10 rounded-full overflow-hidden bg-bgSecondary p-1 flex-shrink-0">
                                            <Image 
                                                src={getProfileUrl(yapper2?.image || "")}
                                                alt=""
                                                width={32}
            ...
            <div className="yapTypeSection w-full">
                <form onSubmit={sendMessage} className="flex gap-2">
                    <input
                        ...
                        className="flex-1 bg-bgPrimary text-fontPrimary rounded-full px-4 py-2 focus:outline-none focus:ring-2 focus:ring-bgSecondary"
                    />
                    <button
                        type="submit"
                        className="bg-bgPrimary text-fontPrimary px-6 py-2 rounded-full hover:bg-[#1b1b1b] transition-colors"
                    >
                        Send
                    </button>
                </form>
